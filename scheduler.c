...
int main(){
    ...
    while(1){
        ...
        if(executing_proc != -1 && proc[executing_proc].exec_time == 0){
		    ...
		    RR_find_head = (executing_proc + 1) % N;
	        executing_proc = -1;
            finished_proc_num ++;
    ...
    if(policy == RR){
        ...
        if(executing_proc != -1){
            ...
            if((clock - last_context_switch_time) % TIME_QUANTEM == 0){ 
                int k = executing_proc;
                do{
                    k ++;
                    k %= N;
                }while(proc[k].ready_time > clock || proc[k].ready_time <= clock && proc[k].exec_time == 0);
                ret = k;
            }
        ...
        /* 若CPU正在閒置，則從RR_find_head開始找到第一個可以跑的process */
        else{
            for(int i = RR_find_head, count = 0; count < N; i ++, count ++){
                i %= N;
